...
## Removed the `/operations` Live Ops screen

Deleted at the user's request: the Vision screen supersedes it, so the Live
Operations Map was a second map maintaining a second set of the same layers.

### What was removed

| Path | Size | Why safe |
|---|---|---|
| `frontend/src/routes/operations.tsx` | 0.6 KB | the route itself |
| `frontend/src/components/ops/LiveOperationsMap.tsx` | 18.3 KB | imported by `operations.tsx` and nothing else |

Six references that would otherwise point at a dead route:

- `Shell.tsx` — voice-command route matcher (`response ops|operations|live ops|live map`)
- `Shell.tsx` — route-title map entry (`"/operations": t("Live Ops")`)
- `Shell.tsx` — sidebar nav item, and the now-unused `Siren` icon import
- `routes/index.tsx` — the "Response Ops" footer link on the landing page
- `input/gestureActions.ts` — `MAP_ROUTES` (gesture pan/zoom targets)
- `input/gestureActions.ts` — `SCREEN_CYCLE` (swipe-to-cycle ring)

Missing any of the last two would have left the gesture layer trying to pan a
route that no longer exists.

### What was deliberately kept

**The three sibling `ops-*` screens.** `/ops-predictive`, `/ops-dispatch` and
`/ops-camera` are separate screens with their own panels (`PredictivePanel`,
`DispatchPanel`, `ReviewPanel`). Only the Live Ops *map* was asked about, and each
of the three was verified still rendering afterwards.

**The entire `/api/ops/*` backend.** Untouched, and it must stay: Vision's snapshot
reads the ops tables for the patrols, dispatches, signals and cameras layers via
`vision_service`, and `openVisionSocket()` connects to `/api/ops/ws` for the live
transport. Deleting the ops API to "finish the job" would have broken four Vision
layers and the live feed. `git status` on `backend/` confirmed clean.

### Noticed while mapping the dependencies

`frontend/src/components/ops/DemoSimPanel.tsx` (11.1 KB) has **zero importers** —
it is already dead code, and was so before this change. Left in place: it is not
part of what was asked for, and deleting unrelated files as a side effect is how a
scoped change turns into an unreviewable one. Flagged here for a separate decision.

### The lint trap avoided

`eslint` reports **392 problems** across `Shell.tsx`, `index.tsx` and
`gestureActions.ts`. Almost all are pre-existing: CRLF line endings and code that
was never run through prettier. Running `--fix` would have reformatted thousands of
unrelated lines and buried a 7-line deletion inside a whole-file rewrite.

Instead the four blank lines left behind by the removals were closed by hand. Final
diff: **1 insertion, 7 deletions across 3 files**, with no collateral reformatting.

### Verified

- `tsc --noEmit` at its **56-error baseline**, with zero errors mentioning
  `operations` or `Siren`.
- `routeTree.gen.ts` regenerated by the TanStack plugin — zero `operations`
  references remain in it.
- Headless Chrome: `/operations` now returns a **not-found page and does not
  crash**; `/ops-predictive`, `/ops-dispatch` and `/ops-camera` all still render;
  `/vision` still renders with **12,584 crime cells**; the sidebar exposes no
