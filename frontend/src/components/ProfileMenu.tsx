...
export function ProfileMenu({ onOpenSettings }: { onOpenSettings: () => void }) {
      ...
      .then((u) => {
        ...
        setAccounts((prev) => {
          ...
          updated[idx] = {
            ...
            name: u.name,
            role: u.rank,
            workspace: u.district || u.range_name || updated[idx].workspace,
          };
          return updated;
        });
  ...
  const displayWorkspace =
    active.id === me?.id ? me?.district || me?.range_name || active.workspace : active.workspace;
  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        <Avatar
          acc={{ ...active, name: displayName, role: displayRank, workspace: displayWorkspace }}
          size="sm"
        />
          ...
          <div className="border-b-2 border-foreground p-2">
            ...
            <div className="flex flex-col gap-1">
              {accounts.map((acc) => {
                ...
                const dWs =
                  acc.id === me?.id
                    ? me?.district || me?.range_name || acc.workspace
                    : acc.workspace;
                return (
                  <button
                    key={acc.id}
            ...
            <div className="flex flex-col items-center gap-3 p-4">
              ...
              {active.photo && (
                <button
                  onClick={() => {
                    setAccounts((p) =>
                      p.map((a) => (a.id === active.id ? { ...a, photo: undefined } : a)),
                    );
                    setPhotoEditor(false);
                  }}
          ...
          >
                ...
                <ul className="mt-3 flex flex-col gap-1.5">
                  {RELOAD_STEPS.map((label, i) => {
                    ...
                    return (
                      <li
                        key={label}
                        className={`flex items-center gap-2 rounded-[5px] border-2 px-2 py-1.5 text-[11px] font-bold ${state === "done" ? "border-foreground bg-primary/15" : state === "active" ? "border-foreground bg-background" : "border-transparent text-foreground/50"}`}
                      >
                        {state === "done" ? (
            ...
            {phase === "confirm" && (
              <div className="flex items-center justify-end gap-2 border-t-2 border-foreground bg-background p-3">
                <button
                  onClick={cancelSwitch}
                  className="rounded-[5px] border-2 border-foreground bg-secondary-background px-3 py-1.5 text-xs font-extrabold nb-shadow-sm hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-none transition"
                >
                  {t("Cancel")}
                </button>
                <button
                  onClick={confirmSwitch}
                  className="flex items-center gap-1.5 rounded-[5px] border-2 border-foreground bg-primary px-3 py-1.5 text-xs font-extrabold text-primary-foreground nb-shadow-sm hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-none transition"
                >
