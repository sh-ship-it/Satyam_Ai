...
export function LiveOperationsMap() {
  ...
  useEffect(() => {
    ...
    active.forEach((a) => {
      ...
      const icon = L.divIcon({
        className: "",
        html: `<div style="position:relative;width:22px;height:22px;display:flex;align-items:center;justify-content:center"><span style="position:absolute;width:22px;height:22px;border-radius:9999px;background:#f9731688;animation:opsmap-pulse 1.6s ease-out infinite"></span><span style="position:relative;width:11px;height:11px;border-radius:9999px;background:#ef4444;border:2px solid #fff"></span></div>`,
        iconSize: [22, 22],
        iconAnchor: [11, 11],
      });
        ...
        for (const z of rz.zones.slice(0, 3)) {
          try {
            ...
            setRoutes((prev) => [
              ...
              {
                id: d.id,
                coords: (d.route ?? []).map(([lng, lat]) => [lat, lng] as [number, number]),
              },
            ]);
          } catch {
            /* no free unit */
    ...
    <div className="absolute inset-0 bg-[#0b0f17]">
      ...
      <div className="pointer-events-none absolute left-3 top-3 z-[1000] flex flex-col gap-2">
        <div className="pointer-events-auto rounded-[8px] border-2 border-foreground bg-background/90 px-4 py-2 backdrop-blur">
          ...
          <div className="mt-1 flex items-center gap-1 text-[11px] font-bold tracking-wide text-muted-foreground">
            ...
            {hotspots.length} {t("CRIME HOTSPOTS")} — {riskCount} {t("RISK CELLS")} —{" "}
            {patrols.length} {t("UNITS")} — {enRouteCount} {t("EN ROUTE")}
          </div>
        </div>
        {corridor && (
      ...
      <div className="absolute right-3 top-3 z-[1000] flex flex-col items-end gap-2">
        ...
        <div className="rounded-[8px] border-2 border-foreground bg-background/90 px-3 py-2 text-[10px] font-bold backdrop-blur">
          ...
          <div className="mb-1 flex items-center gap-1">
            <span className="inline-block h-2.5 w-2.5 rounded-full bg-[#f97316]" /> {t("Incident")}
          </div>
          <div className="mb-1 flex items-center gap-1">
            <span className="inline-block h-2.5 w-2.5 rounded-full bg-[#a855f7]" /> {t("Patrol")}
          </div>
          <div className="flex items-center gap-1">
            <span className="inline-block h-2.5 w-2.5 rounded-full bg-[#00E6A8]" /> {t("Signal")}
          </div>
        </div>
      </div>
    </div>
