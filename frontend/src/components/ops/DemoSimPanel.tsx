...
type Dispatch = {
  id: number; callsign: string; sceneName: string;
  origin: LL; scene: LL;
  route: Hotspot[];
  corridor: [number, number][];
  signals: SimSignal[];
  carIdx: number; car: LL; phase: Phase;
  distanceKm: number; etaSec: number; holdTicks: number;
};
...
export function DemoSimPanel() {
  ...
  function buildDispatches(): Dispatch[] {
    return scenes.slice(0, 5).map((s, i) => {
      ...
      return {
        id: i + 1, callsign: CALLSIGNS[i % CALLSIGNS.length], sceneName: s.name,
        origin, scene, route, corridor, signals: makeSignals(corridor, i + 1),
        carIdx: 0, car: origin, phase: "ACCEPTED" as Phase,
        distanceKm: haversineKm(origin, scene),
        etaSec: (haversineKm(origin, scene) / SPEED_KMH) * 3600, holdTicks: 0,
      };
    });
  }
      ...
      setDispatches((prev) => {
        ...
        const next = prev.map((d) => {
          ...
          if (d.phase === "EN_ROUTE") {
            ...
            return { ...d, carIdx: idx, car: { lat, lng }, etaSec: (haversineKm({ lat, lng }, d.scene) / SPEED_KMH) * 3600 };
          }
          if (d.phase === "ON_SCENE") {
            if (d.holdTicks > 12) { pushFeed(`${d.callsign} cleared — corridor released`); return { ...d, phase: "COMPLETED" as Phase }; }
  ...
  function stopAll() { clearTimer(); setDispatches([]); pushFeed("All simulations stopped"); }

  function toggleDemo() {
    if (demoMode) { stopAll(); setDemoMode(false); } else { setDemoMode(true); }
  }

  const active = dispatches.filter((d) => d.phase !== "COMPLETED");
  const scenePoints: Hotspot[] = dispatches.map((d) => ({ lat: d.scene.lat, lng: d.scene.lng, weight: 2, label: d.sceneName }));
  const routePaths: Hotspot[][] = active.map((d) => d.route);
  const liveMarkers: Hotspot[] = active.filter((d) => d.phase === "EN_ROUTE").map((d) => ({ lat: d.car.lat, lng: d.car.lng, weight: 3, label: `${d.callsign} ${t("en route")}` }));
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <button onClick={simulateAll} disabled={!demoMode}
        ...
        <span className="text-[11px] text-muted-foreground">{t("Runs in-browser — scenes from real forecast hotspots, no backend required.")}</span>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-[260px_1fr_300px]">
        <aside className="flex flex-col gap-2">
          ...
          {dispatches.length === 0 && <p className="text-xs text-muted-foreground">{t("Turn on Demo Mode, then hit Simulate All.")}</p>}
          {dispatches.map((d) => (
            <div key={d.id} className="rounded-[8px] border-2 border-foreground p-2 text-xs">
              <div className="flex items-center justify-between font-bold">
                <span className="inline-flex items-center gap-1"><Truck className="h-3.5 w-3.5" /> {d.callsign}</span>
                <span className="rounded-[4px] border-2 border-foreground px-1 text-[10px]">{d.phase}</span>
              </div>
              <div className="mt-1 text-muted-foreground">{d.sceneName}</div>
              <div className="mt-0.5 text-muted-foreground">{d.distanceKm.toFixed(1)} km · ETA {Math.max(0, Math.round(d.etaSec / 60))} {t("min")}</div>
            </div>
          ))}
        </aside>

        <div className="h-[520px] overflow-hidden rounded-[8px] border-2 border-foreground">
          <CrimeMap points={scenePoints} mode="pins" darkTiles lockBounds={active.length > 0}
            routePaths={routePaths} corridorPath={corridorPath} liveMarkers={liveMarkers} signals={signals} />
        </div>
        ...
        <aside className="flex flex-col gap-3">
          <div className="rounded-[8px] border-2 border-foreground p-3">
            ...
            <p className="mt-1 text-xs text-muted-foreground">{t("Signals prioritized for responding units.")}</p>
            ...
            <ul className="mt-2 flex max-h-[320px] flex-col gap-1 overflow-y-auto text-[11px]">
              ...
              {feed.map((f) => (
                <li key={f.id} className="flex justify-between gap-2 border-b border-foreground/20 pb-1">
                  <span>{f.text}</span><span className="shrink-0 text-muted-foreground">{f.ts}</span>
                </li>
              ))}
            </ul>
          </div>
