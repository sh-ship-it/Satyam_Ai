...
function SocioDashboard() {
  ...
  useEffect(() => {
    ...
    ]).then(([d, c, r]) => { setDemo(d); setCorr(c); setRisk(r); setError(null); })
      .catch(e => setError(e?.status === 403 ? "SP+ rank required to view this data." : "Could not load socio data."))
    ...
    <Shell>
      <div className="flex flex-col h-full overflow-auto">
        ...
        <div className="flex-1 p-6 space-y-6">
                    ...
                    return (
                      <div key={b.bucket} className="flex items-center gap-2 mb-1">
                        ...
                        <div className="flex-1 h-3 rounded-full bg-muted overflow-hidden border border-border">
                          <div className="h-full bg-primary rounded-full" style={{ width: `${(b.count / max) * 100}%` }} />
                        </div>
                  ...
                  {demo.gender.map(g => {
                    ...
                    return (
                      <div key={g.gender} className="flex items-center gap-2 mb-1">
                        ...
                        <span className="w-10 text-right text-xs tabular-nums">{Math.round(g.count / total * 100)}%</span>
                      </div>
                    );
                  })}
                </ChartCard>
          ...
          {corr && (
            <section>
              <h2 className="mb-2 text-sm font-extrabold uppercase tracking-wide">Correlation Matrix</h2>
              ...
              <div className="overflow-hidden rounded-[5px] border-2 border-foreground nb-shadow-sm">
                <table className="w-full text-xs">
                  <thead className="bg-muted/50 text-[11px] uppercase text-muted-foreground">
                    <tr>
                      {["District", "Crime Rate", "Literacy %", "Urban %", "Income Index"].map(h => (
                  ...
                  <tbody className="divide-y divide-border">
                    {corr.scatter.slice(0, 10).map(r => (
                      <tr key={r.district} className="hover:bg-muted/20">
                        ...
                        <td className="px-3 py-2 tabular-nums">{r.income_index?.toFixed(2) ?? "—"}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          {risk && (
            <section>
              <h2 className="mb-3 text-sm font-extrabold uppercase tracking-wide">Social Risk Index</h2>
                ...
                {risk.areas.map(a => (
                  <div key={a.district} className="rounded-[5px] border-2 border-foreground bg-card p-3 nb-shadow-sm">
                    ...
                    <div className="flex flex-wrap gap-1">
                      {a.drivers.map(d => (
                        <span key={d} className="rounded-[3px] bg-muted px-1.5 py-0.5 text-[10px] text-muted-foreground">{d}</span>
                      ))}
                    </div>
                  </div>
                ))}
...
function ChartCard({ title, icon, children }: { title: string; icon: React.ReactNode; children: React.ReactNode }) {
  return (
    <div className="rounded-[5px] border-2 border-foreground bg-card p-4 nb-shadow-sm">
      <div className="flex items-center gap-2 mb-3 text-sm font-bold">{icon}{title}</div>
      {children}
    </div>
  );
}
