...
async function applySceneToEditor(editor: Editor, rawNodes: SceneNode[], rawEdges: SceneEdge[]) {
  ...
  for (const n of nodes) {
    ...
    const fill = (
      ["note", "warning", "decision"].includes(n.entity_kind ?? "") ? "semi" : "solid"
    ) as any;
...
function ShapePalette({ editor }: { editor: Editor }) {
  ...
  async function importAudio(files: FileList | null) {
    ...
    toast.success(
      `Added ${files.length} audio clip${files.length > 1 ? "s" : ""} as evidence cards.`,
    );
  }
...
function TopBar({
  ...
  aiImages: BoardImage[];
  setAiImages: (imgs: BoardImage[]) => void;
  aiLoading: boolean;
  onGenerate: () => void;
  showAI: boolean;
        ...
        <div className="absolute bottom-16 right-4 z-[500] w-80 rounded-[8px] border-2 border-foreground bg-card shadow-2xl pointer-events-auto">
          ...
          <div className="p-2 space-y-2">
              ...
              <div className="flex gap-1 flex-wrap">
                {aiImages.map((img, i) => (
                  <div key={i} className="relative">
                    ...
                    />
                    <button
                      onClick={() => setAiImages(aiImages.filter((_, j) => j !== i))}
                      className="absolute -top-1 -right-1 bg-destructive text-white rounded-full h-4 w-4 flex items-center justify-center text-[9px]"
                    >
                      ×
                    </button>
                  </div>
            ...
            <p className="text-[9px] text-muted-foreground">
              {t("AI builds the scene — use tldraw tools to edit anything.")}
            </p>
          </div>
        </div>
      )}
...
function BoardInner({
  ...
  useEffect(() => {
    try {
      ...
      /* API may differ across patch versions */
    }
  ...
  async function loadBoard(id: number) {
    try {
      ...
      toast.error("Could not open board.");
    }
  }
  ...
  async function handleExport() {
    try {
      ...
      if (ids.length === 0) {
        toast.info("Nothing to export — add some shapes first.");
        return;
      }
...
function BoardScreen() {
  ...
  const [savedBoards, setSavedBoards] = useState<
    { board_id: number; title: string; orphaned?: boolean }[]
  >([]);
