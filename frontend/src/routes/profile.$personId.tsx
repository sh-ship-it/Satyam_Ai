...
import {
  Network, Clock, AlertTriangle, ChevronRight, Search, X,
  User, MapPin, Calendar, FileDown, Shield, Users, Fingerprint,
  TrendingUp, Hash, Loader2, ArrowLeft, Printer,
} from "lucide-react";
...
function PersonSearch({ onSelect }: { onSelect: (r: SearchResult) => void }) {
  ...
  return (
    <div className="relative w-full max-w-xl">
      <div className={`flex items-center gap-2 rounded-xl border-2 bg-background px-4 py-2.5 transition-all
        ...
        <div className="absolute top-full left-0 right-0 mt-1.5 z-50 rounded-xl border border-border bg-card shadow-2xl overflow-hidden divide-y divide-border/60">
          ...
          {results.filter(r => r.type === "person").length > 0 && (
            <>
              ...
              {results.filter(r => r.type === "person").map(r => (
                <button key={`p-${r.id}`} onClick={() => pick(r)}
          ...
          {results.filter(r => r.type === "case").length > 0 && (
            <>
              ...
              {results.filter(r => r.type === "case").map(r => (
                <button key={`c-${r.id}`} onClick={() => pick(r)}
                  className="w-full flex items-center gap-3 px-4 py-2.5 hover:bg-muted/50 text-left transition">
                  <div className="h-8 w-8 rounded-full bg-orange-500/10 flex items-center justify-center shrink-0">
                    <Hash className="h-4 w-4 text-orange-500" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-semibold text-foreground font-mono truncate">{r.label}</div>
                    <div className="text-[11px] text-muted-foreground truncate">{r.sub}</div>
                  </div>
                </button>
              ))}
...
function ProfileScreen() {
  ...
  const [personMeta, setPersonMeta] = useState<{ name: string; gender?: string; age?: number; district?: string } | null>(null);
  const [activeTab, setActiveTab] = useState<"overview" | "history" | "associates" | "mo">("overview");
  ...
  function handleSearchSelect(r: SearchResult) {
    if (r.type === "person") {
      setPersonMeta({ name: r.label, gender: r.gender ?? undefined, age: r.age ?? undefined, district: r.district ?? undefined });
  ...
  return (
    <Shell>
      <div className="flex flex-col h-full overflow-auto bg-background" id="profile-print-root" ref={printRef}>
        ...
        <div className="border-b border-border bg-card px-6 py-4 no-print">
          <div className="flex items-center gap-4 flex-wrap justify-between">
            <div className="flex items-center gap-3">
              {pid > 0 && (
                <button onClick={() => navigate({ to: "/profile/$personId", params: { personId: "0" } })}
                  className="p-1.5 rounded-lg hover:bg-muted text-muted-foreground transition">
                  <ArrowLeft className="h-4 w-4" />
                </button>
              )}
              <div>
                <div className="text-[10px] font-semibold uppercase tracking-widest text-muted-foreground">{t("PS5 · Offender Profile")}</div>
                <h1 className="text-lg font-extrabold text-foreground">{pid > 0 ? displayName : t("Search for a person or case")}</h1>
              </div>
            </div>
        ...
        {!pid && (
          <div className="flex-1 flex flex-col items-center justify-center gap-6 text-center px-6">
            <div className="p-6 rounded-2xl bg-muted/40 border border-border">
              ...
              <h2 className="text-lg font-bold text-foreground mb-2">{t("Find a Person or Case")}</h2>
              <p className="text-sm text-muted-foreground max-w-sm">
                {t("Search by person name, FIR number, crime type, or date. Select a result to view the complete intelligence dossier.")}
              </p>
            </div>
          ...
          <>
            ...
            <div className="mx-6 mt-5 rounded-xl border border-border bg-card overflow-hidden">
              ...
              <div className={`h-1.5 w-full ${
                profile.risk.label === "Critical" ? "bg-destructive" :
                profile.risk.label === "High" ? "bg-orange-500" :
                profile.risk.label === "Medium" ? "bg-yellow-400" : "bg-emerald-500"
              }`} />
              <div className="p-5 grid md:grid-cols-[1fr_auto] gap-6">
                {/* Identity */}
                <div className="flex items-start gap-4">
                  <div className={`h-16 w-16 rounded-xl flex items-center justify-center text-2xl font-extrabold bg-primary/10 text-primary shrink-0 ${RISK_RING[profile.risk.label] || ""}`}>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap items-center gap-3 mt-1.5 text-sm text-muted-foreground">
                      ...
                      {personMeta?.age && <span className="flex items-center gap-1"><Calendar className="h-3.5 w-3.5" />{t("Age")} {personMeta.age}</span>}
                      {personMeta?.district && <span className="flex items-center gap-1"><MapPin className="h-3.5 w-3.5" />{tData("district", personMeta.district, lang)}</span>}
                      <span className="flex items-center gap-1 text-[11px]"><Hash className="h-3 w-3" />ID: {pid}</span>
                    </div>
                    {/* Quick stats */}
                    <div className="flex flex-wrap gap-3 mt-3">
                      {[
                        ...
                        { label: t("Associates"), val: profile.known_associates.length, color: "text-primary" },
                      ].map(s => (
                        <div key={s.label} className="rounded-lg border border-border bg-muted/30 px-3 py-2 text-center min-w-[70px]">
                          <div className={`text-xl font-extrabold tabular-nums ${s.color}`}>{s.val}</div>
                ...
                <div className="flex flex-col items-center justify-center gap-2 border-l border-border pl-6 min-w-[120px]">
                  <div className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">{t("Risk Score")}</div>
                  <div className={`text-5xl font-extrabold tabular-nums ${
                    profile.risk.label === "Critical" ? "text-destructive" :
                    profile.risk.label === "High" ? "text-orange-500" :
                    profile.risk.label === "Medium" ? "text-yellow-500" : "text-emerald-500"
                  }`}>{profile.risk.score}</div>
                  <span className={`rounded-lg px-3 py-1 text-sm font-bold ${RISK_BG[profile.risk.label] || "bg-muted"}`}>
              ...
              {profile.ring_membership && (
                <div className="border-t border-orange-400/40 bg-orange-500/10 px-5 py-2.5 flex items-center gap-2">
                  ...
                  <span className="text-xs text-muted-foreground">· {profile.ring_membership.ring_id}</span>
                </div>
              )}
            </div>
                      ...
                      {profile.risk.breakdown.map(f => (
                        <div key={f.factor} className="flex items-center gap-3">
                          ...
                          <div className="flex-1 h-2.5 rounded-full bg-muted overflow-hidden">
                            <div className="h-full bg-primary rounded-full transition-all" style={{ width: `${(f.score / 30) * 100}%` }} />
                          </div>
                          <span className="w-8 text-right text-xs tabular-nums font-bold">{f.score}</span>
                          <span className="w-40 text-[11px] text-muted-foreground truncate">{f.reason}</span>
                        </div>
                      ))}
                    </div>
                    <p className="mt-3 text-[10px] text-muted-foreground italic flex items-center gap-1">
                      <AlertTriangle className="h-3 w-3" />{t("Decision support only — not predictive policing.")}
                    </p>
                  </div>
                    ...
                    <div className="rounded-xl border border-border bg-card p-5">
                      ...
                      <div className="space-y-2">
                        {timeline.slice(0, 5).map((e, i) => (
                          <div key={i} className="flex items-center gap-3 p-2.5 rounded-lg hover:bg-muted/30 cursor-pointer"
                            onClick={() => setDrawerCaseId(e.case_id)}>
                            <div className="text-[11px] text-muted-foreground w-20 shrink-0 tabular-nums">{e.date?.slice(0, 10) || "—"}</div>
                            <span className={`rounded-full px-2 py-0.5 text-[10px] font-bold shrink-0 ${
                              e.role === "Accused" ? "bg-destructive/10 text-destructive" : "bg-muted text-muted-foreground"}`}>
                              {tData("role", e.role, lang)}
                            </span>
                            <span className="text-xs font-medium flex-1 truncate">{tData("crime_type", e.crime_type || "Unknown", lang)}</span>
                            <span className="text-[10px] text-muted-foreground">{tData("status", e.status || "", lang)}</span>
                    ...
                    .map(([year, events]) => (
                      <div key={year} className="rounded-xl border border-border bg-card overflow-hidden">
                        <div className="flex items-center justify-between px-4 py-2.5 bg-muted/40 border-b border-border">
                          ...
                          <span className="text-[11px] text-muted-foreground">{events.length} {t("cases")}</span>
                        </div>
                        <table className="w-full text-sm">
                          <thead className="bg-muted/20 text-[10px] uppercase tracking-wider text-muted-foreground border-b border-border">
                            <tr>
                              ...
                              <th className="px-4 py-2 text-left font-semibold">{t("Crime Type")}</th>
                          ...
                          <tbody className="divide-y divide-border">
                            {events.map((e, i) => (
                              <tr key={i} className="hover:bg-muted/20 cursor-pointer"
                                onClick={() => setDrawerCaseId(e.case_id)}>
                                <td className="px-4 py-2.5 text-xs text-muted-foreground tabular-nums">{e.date?.slice(0, 10) || "—"}</td>
                                <td className="px-4 py-2.5">
                                  <span className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${
                                    e.role === "Accused" ? "bg-destructive/10 text-destructive" :
                                    e.role === "Victim" ? "bg-blue-500/10 text-blue-600" : "bg-muted text-muted-foreground"}`}>
                                    {tData("role", e.role, lang)}
                                  </span>
                                </td>
                                <td className="px-4 py-2.5 font-medium">{tData("crime_type", e.crime_type || "Unknown", lang)}</td>
                                <td className="px-4 py-2.5 text-xs text-muted-foreground">{tData("status", e.status || "", lang)}</td>
              ...
              {activeTab === "associates" && (
                <div className="rounded-xl border border-border bg-card overflow-hidden">
                  <div className="px-5 py-3 border-b border-border bg-muted/30 flex items-center justify-between">
                    ...
                    <span className="text-xs text-muted-foreground">{profile.known_associates.length} {t("persons")}</span>
                  </div>
                  {profile.known_associates.length > 0 ? (
                      ...
                      {profile.known_associates.map((a, i) => (
                        <div key={a.person_id}
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <div className="text-sm font-semibold text-foreground">{t("Person")} #{a.person_id}</div>
                              <div className="text-[11px] text-muted-foreground">{a.shared_case_count} {t("shared cases")}</div>
                            </div>
                          </div>
                          <div className="flex items-center gap-3">
                            <div className="flex gap-1">
                              {Array.from({ length: Math.min(a.shared_case_count, 5) }).map((_, j) => (
                    ...
                    <div className="p-8 text-center text-sm text-muted-foreground">{t("No known associates found.")}</div>
                  )}
                </div>
              )}
              ...
              {activeTab === "mo" && (
                <div className="space-y-4">
                  <div className="grid sm:grid-cols-2 gap-4">
                    {[
                      { label: t("Top Crime Types"), items: profile.mo_fingerprint.top_crime_types.map(v => tData("crime_type", v, lang)) },
                      { label: t("Legal Sections"), items: profile.mo_fingerprint.top_sections },
                      { label: t("Motives"), items: profile.mo_fingerprint.top_motives.map(v => tData("motive", v, lang)) },
                      { label: t("Typical Time"), items: profile.mo_fingerprint.time_of_day ? [profile.mo_fingerprint.time_of_day] : [] },
                    ].map(g => (
                      <div key={g.label} className="rounded-xl border border-border bg-card p-4">
                        <div className="text-[10px] font-bold uppercase tracking-wide text-muted-foreground mb-3">{g.label}</div>
                        <div className="flex flex-wrap gap-1.5">
                          {g.items.map((item, i) => (
                            <span key={i} className="rounded-lg border border-border bg-muted/50 px-2.5 py-1 text-xs font-medium text-foreground">{item}</span>
                          ))}
            ...
            <div className="hidden print:block px-8 py-6 text-[11pt]" style={{ fontFamily: "Arial, sans-serif" }}>
              {/* KSP letterhead */}
              <div style={{ borderBottom: "3px solid #1a1a2e", paddingBottom: "8pt", marginBottom: "12pt" }}>
                <div style={{ fontSize: "8pt", textTransform: "uppercase", letterSpacing: "0.1em", color: "#555" }}>Karnataka State Police — Confidential Intelligence Report</div>
                <div style={{ fontSize: "18pt", fontWeight: "bold", marginTop: "2pt" }}>Offender Profile Report</div>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: "8pt", color: "#666", marginTop: "6pt" }}>
                  <span>Generated: {new Date().toLocaleString()}</span>
                  <span>Ref: KSP/PROFILE/{pid}/{new Date().getFullYear()}</span>
                </div>
              </div>

              {/* Identity block */}
              <h2 style={{ fontSize: "12pt", fontWeight: "bold", borderBottom: "1px solid #ccc", paddingBottom: "3pt" }}>1. Subject Identity</h2>
                ...
                <tbody>
                  {[
                    ...
                    <tr key={k} style={{ borderBottom: "1px solid #eee" }}>
                      <td style={{ padding: "3pt 6pt", fontWeight: "bold", width: "35%", fontSize: "9pt", backgroundColor: "#f8f8f8" }}>{k}</td>
              ...
              {/* MO fingerprint */}
              <h2 style={{ fontSize: "12pt", fontWeight: "bold", borderBottom: "1px solid #ccc", paddingBottom: "3pt", marginTop: "14pt" }}>2. Modus Operandi (MO) Fingerprint</h2>
              <table style={{ width: "100%", borderCollapse: "collapse", marginTop: "6pt" }}>
                <thead><tr style={{ backgroundColor: "#f0f0f0" }}>
                  <th style={{ padding: "4pt 6pt", fontSize: "9pt", textAlign: "left", border: "1px solid #ddd" }}>Category</th>
                  <th style={{ padding: "4pt 6pt", fontSize: "9pt", textAlign: "left", border: "1px solid #ddd" }}>Values</th>
                </tr></thead>
                <tbody>
                  {[
                    ...
                    <tr key={k} style={{ borderBottom: "1px solid #eee" }}>
                      <td style={{ padding: "3pt 6pt", fontWeight: "bold", fontSize: "9pt", border: "1px solid #ddd", width: "30%" }}>{k}</td>
                      <td style={{ padding: "3pt 6pt", fontSize: "9pt", border: "1px solid #ddd" }}>{v}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              ...
              {/* Full crime history */}
              <h2 style={{ fontSize: "12pt", fontWeight: "bold", borderBottom: "1px solid #ccc", paddingBottom: "3pt", marginTop: "14pt" }}>3. Complete Crime History ({timeline.length} records)</h2>
              <table style={{ width: "100%", borderCollapse: "collapse", marginTop: "6pt" }}>
                <thead><tr style={{ backgroundColor: "#f0f0f0" }}>
                  {["#", "Date", "Role", "Crime Type", "Status", "Case ID"].map(h => (
                    <th key={h} style={{ padding: "4pt 5pt", fontSize: "8pt", textAlign: "left", border: "1px solid #ddd" }}>{h}</th>
                  ))}
                </tr></thead>
                <tbody>
                  {timeline.map((e, i) => (
                    <tr key={i} style={{ borderBottom: "1px solid #eee", backgroundColor: i % 2 === 0 ? "#fff" : "#fafafa" }}>
                      <td style={{ padding: "3pt 5pt", fontSize: "8pt", border: "1px solid #ddd" }}>{i + 1}</td>
                      <td style={{ padding: "3pt 5pt", fontSize: "8pt", border: "1px solid #ddd", fontFamily: "monospace" }}>{e.date?.slice(0, 10) || "—"}</td>
                      <td style={{ padding: "3pt 5pt", fontSize: "8pt", border: "1px solid #ddd", fontWeight: "bold" }}>{e.role}</td>
                      <td style={{ padding: "3pt 5pt", fontSize: "8pt", border: "1px solid #ddd" }}>{e.crime_type || "—"}</td>
                      <td style={{ padding: "3pt 5pt", fontSize: "8pt", border: "1px solid #ddd" }}>{e.status || "—"}</td>
                      <td style={{ padding: "3pt 5pt", fontSize: "8pt", border: "1px solid #ddd", fontFamily: "monospace" }}>{e.case_id}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              ...
              {profile.known_associates.length > 0 && (
                <>
                  <h2 style={{ fontSize: "12pt", fontWeight: "bold", borderBottom: "1px solid #ccc", paddingBottom: "3pt", marginTop: "14pt" }}>4. Known Associates ({profile.known_associates.length})</h2>
                  <table style={{ width: "100%", borderCollapse: "collapse", marginTop: "6pt" }}>
                    <thead><tr style={{ backgroundColor: "#f0f0f0" }}>
                    ...
                    <tbody>
                      {profile.known_associates.map((a, i) => (
                        <tr key={i} style={{ borderBottom: "1px solid #eee" }}>
                          <td style={{ padding: "3pt 5pt", fontSize: "8pt", border: "1px solid #ddd" }}>#{a.person_id}</td>
                          <td style={{ padding: "3pt 5pt", fontSize: "8pt", border: "1px solid #ddd" }}>{a.shared_case_count}</td>
              ...
              {/* Footer */}
              <div style={{ marginTop: "20pt", borderTop: "1px solid #ccc", paddingTop: "8pt", fontSize: "8pt", color: "#777" }}>
                <strong>CONFIDENTIAL</strong> — This report contains synthetic data for demonstration purposes only.
                All person names, FIR numbers, and incident details are computer-generated and do not represent real individuals or events.
                Prepared by Satyam Crime Intelligence System · KSP × Datathon 2026.
              </div>
            </div>
          </>
        )}
