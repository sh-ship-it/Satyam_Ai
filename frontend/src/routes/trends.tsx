...
function TrendsScreen() {
  ...
  const load = () => {
    ...
    Promise.all([
      ...
      setError(null);
    }).catch(() => setError("Could not load trends data."))
    ...
    <Shell>
      <div className="flex flex-col h-full overflow-auto">
        <div className="border-b-2 border-foreground bg-header px-6 py-4 text-header-foreground">
          <div className="text-[11px] uppercase tracking-wider opacity-60">PS3 · MO Clustering</div>
          <h1 className="text-xl font-extrabold tracking-tight">Trends &amp; Patterns</h1>
        </div>

        {/* Filters */}
        <div className="flex gap-3 px-6 py-3 border-b border-border bg-card/60">
          <input value={crimeType} onChange={e => setCrimeType(e.target.value)} placeholder="Crime type filter…"
        ...
        <div className="flex-1 p-6 space-y-6">
          ...
          {(deltas.qoq_percent != null) && (
            <div className="grid grid-cols-2 gap-4">
              <StatCard label="QoQ Change" value={`${deltas.qoq_percent > 0 ? "+" : ""}${deltas.qoq_percent?.toFixed(1)}%`}
                color={deltas.qoq_percent > 0 ? "text-destructive" : "text-success"} />
              {deltas.yoy_percent != null && (
                <StatCard label="YoY Change" value={`${deltas.yoy_percent > 0 ? "+" : ""}${deltas.yoy_percent?.toFixed(1)}%`}
          ...
          {sortedTypes.length > 0 && (
            <section>
              <h2 className="mb-3 text-sm font-extrabold uppercase tracking-wide flex items-center gap-2"><TrendingUp className="h-4 w-4" /> Top Crime Types</h2>
              <div className="space-y-2">
                {sortedTypes.map(([ct, cnt]) => {
                  const max = sortedTypes[0][1];
                  return (
                    <div key={ct} className="flex items-center gap-3">
                      <span className="w-32 text-xs font-bold truncate">{ct}</span>
          ...
          {peaks.length > 0 && (
            <section>
              <h2 className="mb-3 text-sm font-extrabold uppercase tracking-wide">🗓 Seasonal Peaks</h2>
              <div className="grid gap-2 md:grid-cols-3">
                {peaks.map(p => (
                  <div key={p.period} className="rounded-[5px] border-2 border-foreground bg-card p-3 nb-shadow-sm">
                    ...
                    <div className="text-xs text-primary font-bold mt-0.5">+{p.lift_percent}% above baseline</div>
          ...
          {clusters.length > 0 && (
            <section>
              <h2 className="mb-3 text-sm font-extrabold uppercase tracking-wide flex items-center gap-2"><Layers className="h-4 w-4" /> MO Clusters</h2>
              <div className="overflow-hidden rounded-[5px] border-2 border-foreground nb-shadow-sm">
                <table className="w-full text-sm">
                  <thead className="bg-muted/50 text-[11px] uppercase tracking-wider text-muted-foreground">
                    <tr>
                      {["Cluster", "Cases", "Sections", "Action"].map(h => (
