...
export type Suggestion = {
  ...
  response_improve_sec?: number | null;
  status: string;
  reasons: string[];
};
export type Patrol = {
...
export const responseOps = {
  ...
  riskZones: (refresh = false) =>
    opsFetch<{ zones: RiskZone[]; recomputed: boolean; total: number }>(
      `/risk-zones?refresh=${refresh}`,
    ),
  suggestions: () => opsFetch<{ suggestions: Suggestion[]; total: number }>("/suggestions"),
  ...
  simulate: (id: number) =>
    opsFetch<{ ok: boolean }>(`/dispatch/${id}/simulate`, { method: "POST" }),
  activeDispatches: () => opsFetch<{ active: ActiveDispatch[] }>("/dispatch/active"),
  simulateAll: () =>
    opsFetch<{ ok: boolean; started: number }>("/dispatch/simulate-all", { method: "POST" }),
  ...
  confirmReview: (id: number, autoDispatch = true) =>
    opsFetch<{ ok: boolean; case_id: number; dispatch_id: number | null }>(
      `/review-queue/${id}/confirm?auto_dispatch=${autoDispatch}`,
      { method: "POST" },
    ),
  rejectReview: (id: number) =>
    opsFetch<{ ok: boolean }>(`/review-queue/${id}/reject`, { method: "POST" }),
  clearQueue: () => opsFetch<{ ok: boolean }>("/review-queue/clear", { method: "POST" }),
  corridorState: () =>
  ...
  demoActive: () =>
    opsFetch<{
      active: { dispatchId: number; lat: number; lng: number; status: string; eta_sec: number }[];
    }>("/demo/active"),
  ...
  cameraStart: (video = "frontend/public/total fight.mp4", camera_id = "CAM-001") =>
    opsFetch<{ ok: boolean; status: string; pid?: number | null; stream_port?: number }>(
      `/camera/start?video=${encodeURIComponent(video)}&camera_id=${encodeURIComponent(camera_id)}`,
      { method: "POST" },
    ),
  cameraStop: () => opsFetch<{ ok: boolean; status: string }>("/camera/stop", { method: "POST" }),
  cameraStatus: () =>
    opsFetch<{ running: boolean; pid?: number | null; stream_port?: number }>("/camera/status"),
};
