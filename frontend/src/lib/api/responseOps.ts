...
export const responseOps = {
  ...
  cameraStart: (video = "frontend/public/total fight.mp4", camera_id = "CAM-001") =>
    opsFetch<{ ok: boolean; status: string; pid?: number | null; stream_port?: number }>(`/camera/start?video=${encodeURIComponent(video)}&camera_id=${encodeURIComponent(camera_id)}`, { method: "POST" }),
  cameraStop: () => opsFetch<{ ok: boolean; status: string }>("/camera/stop", { method: "POST" }),
  cameraStatus: () => opsFetch<{ running: boolean; pid?: number | null; stream_port?: number }>("/camera/status"),
};
